...
# def get_cards_from_result_json(result_json) -> list[YugiohCard]:
#     cards: list[YugiohCard] = []

#     for item_key, item_result in result_json.items():
#         yugipedia_result = item_result["printouts"]
#         card = YugiohCard.get_card_from_yugipedia_semantic_search(
#             item_key, yugipedia_result)
#         cards.append(card)
#     return cards


# def get_yugioh_cards_per_semantic_card_search_per_character(character_to_check) -> list[YugiohCard]:
#     LIMIT = 250
#     offset = 0
#     counter = 0

#     obj: dict[str, str] = card_semantic_search_params(
#         character_to_check, offset, LIMIT)

#     card_list: list[YugiohCard] = []
#     total_count = 0
#     max_iterations = 100  # Maximum number of iterations to prevent infinite loops

#     try:
#         res_json = run_wiki_request_until_response(
#             SEMANTIC_URL, header=HEADERS, params=obj)
#         if res_json is not None and res_json:
#             count = len(res_json["results"].keys())
#             if "results" in res_json.keys():
#                 card_list.extend(
#                     get_cards_from_result_json(res_json["results"]))
#                 total_count = count

#                 # Add debug statements to track offset and count
#                 print(
#                     f"Iteration {counter}: Retrieved {count} cards, Offset: {offset}")
#                 if count < LIMIT:
#                     print(
#                         f"Iteration {counter}: Retrieved {count} cards, BUT LESS THAN {LIMIT}, exit loop.")
#                     pass
#                 # Safeguard loop to prevent infinite calls
#                 while count == LIMIT and counter < max_iterations:
#                     # Increment offset by LIMIT instead of `count`
#                     offset += LIMIT
#                     obj = card_semantic_search_params(
#                         character=character_to_check, offset=offset, limit=LIMIT)

#                     try:
#                         res_json = run_wiki_request_until_response(
#                             SEMANTIC_URL, HEADERS, obj)
#                         counter += 1
#                         print("counter", counter)
#                         if res_json and "results" in res_json:
#                             new_cards = get_cards_from_result_json(
#                                 res_json["results"])
#                             card_list.extend(new_cards)
#                             count = len(new_cards)

#                             # Check if the new count is zero; if so, break out
#                             if count == 0:
#                                 print(
#                                     f"Iteration {counter}: No new cards found at offset {offset}.")
#                                 break
...
#                         else:
#                             print(
#                                 f"Unexpected response at iteration {counter}: {res_json}")
#                             break

#                     except requests.exceptions.JSONDecodeError as e:
#                         print(
#                             f"JSON Error - Iteration {counter} based on interval {LIMIT}: {e}")
#                         return card_list
...
#     print(
#         f"Iteration {counter}: Total length of results for {character_to_check}: {total_count}")
#     return card_list
